...
class WorkerMonitor:
    ...
    def _display_workers(self):
        """Display current worker status in single line"""
        if not self._display_active:
            return

        # Build single line with all worker info
        worker_parts = []
        for worker_id in range(1, self.max_workers + 1):
            ...
            if len(clean_status) > 25:
                clean_status = clean_status[:22] + "..."

            # Format: W1:status [STATE]
            worker_part = f"W{worker_id}:{clean_status} {state_display}"
            worker_parts.append(worker_part)

        # Combine all workers and add timestamp
        current_time = time.strftime("%H:%M:%S")
        full_line = " | ".join(worker_parts) + f" | {current_time}"

        # Pad to clear previous content (make sure it's long enough)
        if len(full_line) < 120:
            full_line += " " * (120 - len(full_line))

        # Print with carriage return to overwrite same line
        print(f"\r{full_line}", end="")
        sys.stdout.flush()
